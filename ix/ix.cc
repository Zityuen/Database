...
//	ixfileHandle.readPage(ixfileHandle.rootNodePage, pageData);
	(*(this->ixfileHandle)).readPage((*(this->ixfileHandle)).rootNodePage, pageData);
...
//			cout<<"slotIndex:"<<slotIndex<<"  nextNodePage"<<nextNodePage<<endl;
		...
		if(lowSlotID == -1){
			...
			if(ix->keyCompare(attribute, lowKey, lowKeyLength, key, rOffset.length)==0){
...
//			cout<<rightNodePage<<endl;
		...
		if(highKey == NULL){
			...
			if(highSlotID != -1){
				...
				memcpy(key, (char *)pageData+rOffset.offset, rOffset.length);
				if(ix->keyCompare(attribute, highKey, highKeyLength, key, rOffset.length)==0){
